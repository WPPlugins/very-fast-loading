...
add_action( 'admin_init', 'fastloadingsettings' );

function fastloadingsettings() {
...
function callbackglobals($buffer){
	...
	$search = array('/\>[^\S ]+/s', '/[^\S ]+\</s',	'/(\s)+/s',	 '~>\s+<~', '/\r|\n/', );			
	$replace2 = array('>', '<', '\\1', '><', ' ');		
		
	
	$buffer = !isset($_REQUEST['debug']) ?  trim(preg_replace($search, $replace2, $buffer)) : $buffer;
	#$buffer = !isset($_REQUEST['debug']) ?  preg_replace('/(?:(?:\/\*(?:[^*]|(?:\*+[^*\/]))*\*+\/)|(?:(?<!\:|\\\|\')\/\/.*))/', '', $buffer) : $buffer;		
	$final = strlen($buffer);
	$savings = round((($initial-$final)/$initial*100), 3);
	$buffer .= PHP_EOL . '<!--' . PHP_EOL . '*** This site runs Very Fast Loading plugin v1.0.0 - http://www.cartaddon.com/products/Fast-Loading-Wordpress.html ***' . PHP_EOL . '*** Total size saved: ' . $savings . '% | Size before minify: ' . $initial . ' bytes | Size after minified: ' . $final . ' bytes. ***' . PHP_EOL . '-->';
	return $buffer;
}
ob_start("callbackglobals");
